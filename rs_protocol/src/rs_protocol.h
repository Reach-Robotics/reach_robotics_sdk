/**
 * @file rs_protocol.h
 * @brief Header file for the Reach Robotics SDK protocol implementation.
 *
 * This file contains the definitions and functions for encoding and decoding
 * packets, as well as utility functions for handling data serialization and
 * deserialization. It includes support for COBS (Consistent Overhead Byte
 * Stuffing) encoding/decoding, CRC-8 checksum calculation, and float-to-byte
 * conversions.
 *
 * Key Features:
 * - Packet structure definition for communication.
 * - Functions for encoding and decoding packets.
 * - Utility functions for handling floats and CRC-8 checksums.
 *
 * Usage:
 * - Include this header file in your project to use the protocol functions.
 * - Refer to the function documentation for details on usage and parameters.
 *
 * Note:
 * - Ensure that the `packetID.h` file is included in the project for proper
 *   functionality.
 *
...
/**
 * @brief Calculate the CRC-8 checksum.
 *
 * returns the CRC-8 of data[0..len-1] applied to the seed CRC.
 * It allows the calculation of a CRC one chunk at a time, using the previously
 * returned value as the next seed. If the data pointer is NULL, the function
 * returns the initial seed. Refer to the test code for an example of proper
 * usage.
 */
